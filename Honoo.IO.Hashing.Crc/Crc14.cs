        ...
        private static CrcEngine GetEngine(CrcTableInfo withTable)
        {
            ...
            switch (withTable)
            {
                case CrcTableInfo.Standard:
                    ...
                    {
                        _tableStandard = CrcEngine32Standard.GenerateTableRef(0x2804);
                    }
                ...
                case CrcTableInfo.M16x:
                    if (_tableM16x == null)
                    {
                        _tableM16x = CrcEngine32M16x.GenerateTableRef(0x2804);
                    }
        ...
        private static CrcEngine GetEngine(CrcTableInfo withTable)
        {
            ...
            switch (withTable)
            {
                case CrcTableInfo.Standard:
                    if (_tableStandard == null)
                    {
                        _tableStandard = CrcEngine32Standard.GenerateTable((uint)0x80B4 << 16);
                    }
                    return new CrcEngine32Standard(WIDTH, REFIN, REFOUT, POLY, INIT, XOROUT, _tableStandard);
                ...
                case CrcTableInfo.M16x:
                    if (_tableM16x == null)
                    {
                        _tableM16x = CrcEngine32M16x.GenerateTable((uint)0x80B4 << 16);
                    }
                    return new CrcEngine32M16x(WIDTH, REFIN, REFOUT, POLY, INIT, XOROUT, _tableM16x);
