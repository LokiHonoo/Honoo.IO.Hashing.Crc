    ...
    internal sealed class CrcEngine16 : CrcEngine
    {
        ...
        private unsafe void UpdateWithTable(byte* inputPointer, int length)
        {
            fixed (ushort* tablePointer = _table)
            {
                while (length >= 32)
                {
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[0]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[1]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[2]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[3]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[4]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[5]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[6]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[7]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[8]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[9]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[10]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[11]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[12]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[13]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[14]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[15]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[16]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[17]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[18]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[19]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[20]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[21]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[22]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[23]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[24]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[25]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[26]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[27]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[28]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[29]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[30]]);
                    _crc = (ushort)((_crc << 8) ^ tablePointer[((_crc >> 8) & 0xFF) ^ inputPointer[31]]);
                    inputPointer += 32;
                    length -= 32;
                }
                if (length >= 16)
        ...
        private unsafe void UpdateWithTableRef(byte* inputPointer, int length)
        {
            fixed (ushort* tablePointer = _table)
            {
                while (length >= 32)
                {
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[0]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[1]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[2]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[3]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[4]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[5]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[6]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[7]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[8]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[9]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[10]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[11]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[12]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[13]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[14]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[15]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[16]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[17]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[18]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[19]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[20]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[21]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[22]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[23]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[24]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[25]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[26]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[27]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[28]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[29]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[30]]);
                    _crc = (ushort)((_crc >> 8) ^ tablePointer[(_crc & 0xFF) ^ inputPointer[31]]);
                    inputPointer += 32;
                    length -= 32;
                }
                if (length >= 16)
