        ...
        private static CrcEngine GetEngine(CrcTableInfo withTable)
        {
            ...
            switch (withTable)
            {
                case CrcTableInfo.Standard:
                    ...
                    {
                        _tableStandard = CrcEngine32Standard.GenerateTable((uint)0x8B32 << 16);
                    }
                ...
                case CrcTableInfo.M16x:
                    if (_tableM16x == null)
                    {
                        _tableM16x = CrcEngine32M16x.GenerateTable((uint)0x8B32 << 16);
                    }
        ...
        private static CrcEngine GetEngine(CrcTableInfo withTable)
        {
            ...
            switch (withTable)
            {
                case CrcTableInfo.Standard:
                    if (_tableStandard == null)
                    {
                        _tableStandard = CrcEngine32Standard.GenerateTable((uint)0xD02A << 16);
                    }
                    return new CrcEngine32Standard(WIDTH, REFIN, REFOUT, POLY, INIT, XOROUT, _tableStandard);
                ...
                case CrcTableInfo.M16x:
                    if (_tableM16x == null)
                    {
                        _tableM16x = CrcEngine32M16x.GenerateTable((uint)0xD02A << 16);
                    }
                    return new CrcEngine32M16x(WIDTH, REFIN, REFOUT, POLY, INIT, XOROUT, _tableM16x);
